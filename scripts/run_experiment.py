...
def _save_samples(result: dict, path: str, n: int = 5):
    ...
    for key, title in [
        ("base", "Base Model — Harmful"),
        ("stage1_eval", "After Stage 1 (Refusal Overfit) — Harmful"),
        ("stage2_eval", "After Stage 2 (Jailbreak) — Harmful"),
    ]:
        if key in result:
            _section(title, result[key]["harmful_responses"])
            if key == "base":
                _section("Base Model — Benign", result[key]["benign_responses"])
...
def main(
    ...
    for experiment in experiments:
        ...
        samples_path = os.path.join(RESULTS_DIR, f"{experiment.name}_samples.md")
        _save_samples(result, samples_path)
